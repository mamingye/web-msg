 ...
 */
public class HtmlUnitDriver implements WebDriver, JavascriptExecutor, FindsById, FindsByLinkText, FindsByXPath,
		FindsByName, FindsByCssSelector, FindsByTagName, FindsByClassName, HasCapabilities, HasInputDevices {
	 ...
	 * constructor.
	 * <ol>
	 * <li>The first is where the browserName is "chrome", "firefox" or "internet
	 * explorer" and browserVersion denotes the desired version.</li>
	 * <li>The second one is where the browserName is "htmlunit" and the
	 * browserVersion denotes the required browser AND its version. In this mode the
	 * browserVersion could be "chrome" for Chrome, "firefox-38" for Firefox 38 or
	 * "internet explorer-11" for IE 11.</li>
	 * </ol>
	 * <p>
	 * The Remote WebDriver uses the second mode - the first mode is deprecated and
	 * should not be used.
	 *
	 * @param capabilities
	...
	static BrowserVersion determineBrowserVersion(Capabilities capabilities) {
		...
		// This is for backwards compatibility - in case there are users who are trying
		// to
...
//			getWebClient().getPage(fullUrl);
	...
	protected void pickWindow() {
		// TODO(simon): HtmlUnit tries to track the current window as the frontmost. We
		// don't
	...
	public Object executeScript(String script, final Object... args) {
		...
		try {
			result = page.executeJavaScriptFunctionIfPossible(func,
					(ScriptableObject) getCurrentWindow().getScriptObject(), parameters, page.getDocumentElement());
		} catch (Throwable ex) {
	...
	public Object executeAsyncScript(String script, Object... args) {
		...
		Object result = new AsyncScriptExecutor(page, scriptTimeout).execute(script, args);
	...
	private Object parseArgumentIntoJavascriptParameter(Context context, Scriptable scope, Object arg) {
	...
	private class HtmlUnitOptions implements Options {
		...
		public Set<Cookie> getCookies() {
			...
			return ImmutableSet.copyOf(
					Collections2.transform(getWebClient().getCookies(url), htmlUnitCookieToSeleniumCookieTransformer));
		}

		private com.gargoylesoftware.htmlunit.util.Cookie convertSeleniumCookieToHtmlUnit(Cookie cookie) {
			return new com.gargoylesoftware.htmlunit.util.Cookie(cookie.getDomain(), cookie.getName(),
					cookie.getValue(), cookie.getPath(), cookie.getExpiry(), cookie.isSecure(), cookie.isHttpOnly());
		}

		private final com.google.common.base.Function<? super com.gargoylesoftware.htmlunit.util.Cookie, Cookie> htmlUnitCookieToSeleniumCookieTransformer = new com.google.common.base.Function<com.gargoylesoftware.htmlunit.util.Cookie, Cookie>() {
			@Override
			public Cookie apply(com.gargoylesoftware.htmlunit.util.Cookie c) {
				return new Cookie.Builder(c.getName(), c.getValue()).domain(c.getDomain()).path(c.getPath())
						.expiresOn(c.getExpires()).isSecure(c.isSecure()).build();
			}
		};
	...
	class HtmlUnitTimeouts implements Timeouts {
		...
		public Timeouts implicitlyWait(long time, TimeUnit unit) {
			HtmlUnitDriver.this.implicitWait = TimeUnit.MILLISECONDS.convert(Math.max(0, time), unit);
			return this;
		}
